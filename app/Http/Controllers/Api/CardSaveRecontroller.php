...
class CardSaveRecontroller extends Controller
{
    public function checkCard(Request $request)
    {
        ...
        if ($card) {
            ...
            return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN', 'data'=> $card, 'success' => false , 'error_type'=> 'Not Found', 'message' => '( saving ) Cannot Procceed Card Not Found',  ], 404);
        }
    }
    ...
    public function processCard($card, $day, $request)
    {
        ...
        if ($card->status == 'Active') {
            ...
            return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN', 'data'=> $card, 'success' => false , 'error_type'=> 'Card Not Active', 'message' => '( saving ) Cannot Procceed Card is ' . $card->status], 404);
        }
    }
    ...
    public function checkCardValidity($card, $day,  $request)
    {
        ...
        if ($isCardValid) {
            ...
            // return response()->json(['error' => 'Card is Validity Only in' , 'success' => false'transaction'=> 'saving','source'=> 'USM-ADMIN'], 404);
        

            
            return response()->json([
                'transaction'=> 'saving','source'=> 'USM-ADMIN', 
                'data'=> $card, 'success' => false , 'error_type'=> 'Card Expired', 
                'message' => '( saving ) Cannot Procceed Card is expired. The validity of the card is valid only from ' . $cardValidFromStartOfDay->format('F j, Y') . ' until ' . $cardValidUntilEndOfDay->format('F j, Y') . ' based on the date set in the setting from ' . $cardSettingValidFromStartOfDay->format('F j, Y'), ], 404);
            ...
            //     'success' => false
            // 'transaction'=> 'saving','source'=> 'USM-ADMIN'], 404);
        }
    ...
    public function checkCardlatestRecord($card, $day, $request)
    {
        ...
        if ($request->scanned == 'entry') {
            ...
            return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN', 'data'=> $card, 'success' => false , 'error_type'=> 'No Scanned Parameter When Sending API', 'message' => '( saving ) Cannot Procceed  Cant Identify Whic Side of the door scanned missing parameter' ], 404);
            // return response()->json(['error' => 'scanned type no value', 'success' => false'transaction'=> 'saving','source'=> 'USM-ADMIN'], 404);
        }
    }
        ...
        if (!empty($card_latest_record)) {
            ...
            if ($today->equalTo($latest_day_record_date)) {
                ...
                if($card_latest_record->entry == false  && $card_latest_record->exit == false){
                    ...
                    return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN', 'data'=> $card, 'success' => true , 'error_type'=> null, 'message' => '( saving ) Success! Card Doesnt Have Entry Record']);
                    
                }else if($card_latest_record->entry == true  && $card_latest_record->exit == false){
                    
                    return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN', 'data'=> $card, 'success' => false , 'error_type'=> 'Already Login', 'message' => '( saving ) Cannot proceed. Card cannot enter again until it has exited',], 404);
                    
                }else if($card_latest_record->entry == false  && $card_latest_record->exit == true){

                    return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN', 'data' => $card, 'success' => false, 'error_type' => 'Invalid Exit', 'message' => '( saving ) Cannot Procceed Invalid exit without entry',], 404);
                }
                
                else{
                    return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN', 'data'=> $card, 'success' => true , 'error_type'=> null, 'message' => '( saving ) Success! Card Ready To Login Again']);
                ...
                return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN', 'data'=> $card, 'success' => true , 'error_type'=> null, 'message' => '( saving ) Success! Card last Record is Not The Same Today']);
                // return response()->json(['data' => $card, 'success' => true, 'request' => 'last record is not the same today']);
            }

        } else {
            return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN', 'data'=> $card, 'success' => true , 'error_type'=> null, 'message' => '( saving ) Success! Card Doesnt Have Record ']);
    ...
    public function cardProcessForExit($card, $day, $request)
    {
        ...
        if (!empty($card_latest_record)) {
            if ($card_latest_record->exit == false) {
                ...
                return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN','data' => $card, 'success' => true, 'error_type' => null, 'message' => '( saving ) Ready To Exit Because It nit exit yes']);
            } else {
                // Card has already exited on the same day
                return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN','data' => $card, 'success' => false, 'error_type' => 'Multiple Exit ', 'message' => '( saving ) Cannot Exit Over and Over Again. Enter first.', ], 404);
            }
        } else {
            // No records for the card yet, handle accordingly
            return response()->json(['transaction'=> 'saving','source'=> 'USM-ADMIN','data' => $card, 'success' => true, 'error_type' => null, 'message' => '( saving ) No entry record found for the card']);
